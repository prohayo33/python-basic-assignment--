...
--최종 조인 결과의 전체 행 수 확인하기 
SELECT count(*)AS join_result
FROM orders o JOIN customers c ON o.customer_id = c.customer_id  
LEFT JOIN shipments s ON o.order_id = s.order_id;

-- shipment_id가 null인 행 수 조회하기 (where절에)
SELECT count(*)AS shipment_is_null
FROM orders o JOIN customers c ON o.customer_id = c.customer_id  
LEFT JOIN shipments s ON o.order_id = s.order_id
WHERE shipment_id IS NULL; 


-- shipment_id가 null인 행 수 조회하기(on에)
SELECT o.order_id, s.shipment_id 
FROM orders o JOIN customers c ON o.customer_id = c.customer_id  
LEFT JOIN shipments s ON o.order_id = s.order_id AND shipment_id IS null; 

--8. order_id별 행 수를 GROUP BY하고, 2행 이상 나타나는 주문이 있는지 확인하세요.
SELECT o.order_id,count(*)AS order_id_count
FROM orders o JOIN customers c ON o.customer_id = c.customer_id  
LEFT JOIN shipments s ON o.order_id = s.order_id
GROUP BY o.order_id 
...
/*
 9. 다음 질문에 답하세요.
     Q1. shipments를 INNER JOIN이 아니라 LEFT JOIN으로 연결해야 하는 이유는 무엇인가요?
			-> Inner join을 하게 되면 shipments에 없는 값은 삭제되어 배송 정보가 없는 주문이 결과에서
			보이지 않게 된다. left join을 하게 되면 orders 테이블에 정보가 있으면 shipments 테이블에
			정보가 없더라도 남아 있을 수 있기 때문이다.
     Q2. shipment_id가 NULL인 행은 어떤 의미인가요?
     	-> shipment_id는 없지만 orders 정보는 있다는 의미이다.
     Q3. 조인 후 행 수가 orders보다 많아졌다면 어떤 관계나 데이터를 먼저 점검해야 하나요?
     	-> 조인 후 행 수가 orders보다 많아졌면 1:N 인지 N:M 관계인지를 확인해 보아야 하며, 관계가 선택적인 경우 null을 
     	어떻게 다룰지 점검해야 한다
     Q4. 조인 결과 검증을 위해 행 수, 중복, NULL을 함께 확인해야 하는 이유는 무엇인가요?
     	-> 행 수를 비교해 예상치 못한 증가가 있는지 확인하고, 중복 여부 확인으로 어떤 기준 키가 join후 여러 행으로 확장 되었는지
     	확인한다. 행이 증가했다고 반드시 중복 오류는 아니기 때문에 중복도 확인해야 한다. left join을 사용한 컬럼에서
     	의도한 대로 null이 나타나는지 의도치 않게 null이 발생하는지 확인을 통해 제대로 join이 되었는지를 알 수 있다.
     Q5. LEFT JOIN한 shipments의 status 조건을
         ON 절에 작성하는 경우와 WHERE 절에 작성하는 경우
         결과가 어떻게 달라질 수 있나요?
         -> where에 작성하는 경우는 이미 조인이 끝난 결과에서 어떤 데이터를 남길지 조건을 지정하는 것으로 shipment_id가 
         null이 아닌 행은 where 단계에서 제외된다. 
         on절에 작성하는 것은 어떤 데이터를 연결할지 조건을 지정하는 것으로 shipment_id가 null이 아니더라도 행 자체는 유지되고
         대신 shipment_id 값이 다 null로 들어간다.
