...
5. 다음 질문에 답하세요.
   ...
   -> actual time, actual rows, loops처럼 실제 실행 결과 확인 가능, Execution time도 확인 가능
   Q2. cost는 실제 실행 시간(ms)인가요? 
   -> 실제 실행 시간 아니다. 옵티마이저가 실행계획 후보를 비교하기 위해 사용하는 상대적 예상 비용 
...
6. 다음 질문에 답하세요.
   ...
   Q3. orders와 customers의 Scan 노드에 표시된 actual rows와
       ...
       -> Rows Removed by Filter = Scan과정에서 확인했지만 필터 조건을 만족하지 않아 제외된 행 수
...
5. 다음 항목을 확인해 기록하세요.
   ...
   - 예상 rows: 60
   - actual rows: 48
   - Rows Removed by Filter: 1999984
   - Execution Time: 115.073 ms
   
6. 예상 rows와 actual rows를 비교하여
   옵티마이저의 예상이 실제 결과와 어느 정도 일치하는지 설명하세요.
   예상 rows는 60이었고 실제 rows는 48이었다. 
   따라서 옵티마이저의 예상과 실제 예상사이에는 12정도 차이가 있었다.

7. 다음 질문에 답하세요.
   Q1. 가장 먼저 확인해야 할 데이터 접근 방식은 무엇인가요? 스캔 방식이다.
   Q2. Seq Scan은 테이블의 데이터를 어떤 방식으로 확인하나요? 테이블 처음부터 끝까지 확인한다.
