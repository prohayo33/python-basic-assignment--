...
8. 데이터 분포만을 기준으로 보았을 때
   어느 컬럼에 인덱스를 추가하는 것이 더 효과적일 가능성이 높은지 판단하고 이유를 작성하세요.
	- customer_id이다. (카디널리티 높고, 선택도 낮으니까)
...
--2,3 조건 만족 행수랑 선택도(selectivity)
SELECT count(*) AS satisfied_31428, round(count(*) * 100.0 / (SELECT count(*) FROM orders), 6) AS selectivity_pct
FROM orders o 
WHERE customer_id = 31428;

-- 4.
SELECT count(*), count(DISTINCT status) AS status_cardinality
FROM shipments s ;

--5. 
SELECT status, count(*) AS total_rows, round(count(*) * 100.0 / (SELECT count(*) FROM shipments), 2) AS shipments_selectivity
FROM shipments s 
GROUP BY s.status 
ORDER BY status;
...
SELECT *
FROM orders o 
WHERE o.customer_id = 31428;


EXPLAIN ANALYZE 
SELECT *
FROM orders o 
WHERE o.customer_id + 0 = 31428;
...
-- city별 행 수와 전체에서 차지하는 비율
SELECT city, count(*)AS city_row_count, 
round(count(*)*100.0 /(SELECT count(*) FROM  stores),6) AS city_ratio
FROM  stores
GROUP BY city;

-- 기존 idx_stores_city 인덱스 삭제
...
-- 인덱스 생성 전 후 비교
--   - 스캔 방식: Seq Scan으로 동일하다. 
--   - 조건이 표시되는 위치(Index Cond 또는 Filter): filter
--   - cost: 0.00..2.25로 동일하다.
--   - actual rows: 31로 동일
--   - Execution Time: 0.027ms, 0.023ms로 크게 차이가 나지 않는다.


/* 
- 네 가지 기준으로 stores.city 인덱스의 적절성 판단하기
   - 카디널리티: city 열의 카디널리티는 4로 높지 않다.
   - 선택도: city행에서 Mumbai의 선택도는 31%정도로 선택도가 낮은 편은 아니다.
   - 테이블 크기: 테이블 크기는 100행 정도로 작다.
   - 인덱스 유지 비용: 테이블이 충분히 작기 때문에 조회 비용이 크지 않다. 따라서 INDEX를 유지하는 비용이 조회 비용을 줄이는 것보다 더 크다.
  
- 최종적으로 "인덱스 추가를 적극적으로 권장한다 / 우선순위가 낮다" 중 하나를 선택하고 근거를 작성하시오.
  인덱스 우선순위가 낮다. 
  앞서 살펴본 것처럼 city 열은 cardinality가 4로 작고, 선택도도 약 31%로 조건에 해당하는 행이 비교적 많다. 
  또한 테이블의 크기가 매우 작기 때문에 인덱스를 통해 데이터를 찾는 과정에서 발생하는 비용이 테이블 전체를 조회하는 것보다 높아질 가능성이 있다.
  따라서 city 열에 인덱스를 추가하더라도 조회 성능 향상 효과가 크지 않을 것으로 판단되며, 옵티마이저가 인덱스보다 Full Table Scan을 선택할 가능성이 높다.
   */



-- 실습 종료 후 idx_stores_city 인덱스를 삭제하세요.
...
1. 카디널리티가 높고 선택도가 낮은 조건에서 인덱스가 유리한 이유는 무엇인가요?
	- 서로 다른 값이 많고 조건을 만족하는 행이 적으면 인덱스를 통해 전체 테이블을 읽지 않고 필요한 소수의 행 위차만 빠르게 찾을 수 있음
	
2. 인덱스가 존재해도 PostgreSQL이 Seq Scan을 선택할 수 있는 이유는 무엇인가요?
	- 옵티마이저는 인덱스 존재 여부만 보는 것이 아니라
	조회해야 할 행의 비율, 테이블 크기, 통계정보 등을 이용해 예상 비용을 비교
...
4. 인덱스 추가 여부를 판단할 때 확인해야 할 네 가지 기준은 무엇인가요?
	- 1. 컬럼의 카디널리티가 충분히 높은가 
	- 2. 조건을 적용했을 때 선택도가 낮은가?
	- 3. 테이블 규모가 충분히 큰가?
	- 4. 조회 이득이 INSERT, DELETE, UPDATE 시 발생하는 인덱스 유지 비용을 감당할 만큼 큰가?
*/
