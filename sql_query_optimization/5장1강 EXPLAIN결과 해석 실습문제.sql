...
10. 다음 질문에 답하세요.
    ...
    -> rank()가 partion by 고객_id, order by 부분에 있기 때문에
...
SELECT p.customer_id, p.order_id, p.product_id, p.price, rank() OVER(PARTITION BY customer_id ORDER BY p.price DESC )AS customer_per_rank
FROM order_item_price p CROSS JOIN avg_price a
...
SELECT *
FROM orders o 
WHERE o.order_date >= date '2023-01-01' AND order_date < date '2024-01-01'
