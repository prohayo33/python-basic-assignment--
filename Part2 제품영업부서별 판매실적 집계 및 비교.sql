...
SELECT sales.product_id, product_name, count(sale_id) transaction_count, 
sum(quantity) total_quantity, sum(sale_amount) total_Sales_amount, round(avg(sale_amount), 2) avg_sales_amount
...
SELECT s.sales_dept_id, sales_dept_name, sales_region, 
count(sale_id) transaction_count, sum(quantity) total_quantity, 
sum(sale_amount) total_sales_amount, round(avg(sale_amount),2) avg_sales_amount
FROM sales s JOIN sales_departments sd ON s.sales_dept_id = sd.sales_dept_id 
WHERE sale_date >= '2025-04-01' AND sale_date < '2025-07-01' AND sales_type != '반품'
GROUP BY s.sales_dept_id , sales_dept_name, sales_region
...
총 판매금액이 가장 높은 제품은 리피다운정 20mg이고 영어 부서는 호남영업팀입니다.
상위 그룹은 거래 건수가 하위 그룹에 비해 높으며 총 판매금액 역시 높다.
상위 제품 부서는 거래 건수뿐 아니라 평균 판매금액 역시 하위 그룹보다 높게 나타나 거래 건수와 평균 판매금액 모두 실적 차이에 영향을 준 것으로 보인다. 
다만 거래 건수는 비슷한데 평균 판매금액 차이로 총액이 갈리는 사례도 있어 단가가 실적 차이에 기여했을 가능성이 있다.
