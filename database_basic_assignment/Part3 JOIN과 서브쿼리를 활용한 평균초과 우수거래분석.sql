...
SELECT sale_id, sale_date, product_name, customer_name, customer_type, customer_region, sales_type, quantity, sale_amount, payment_status
FROM public.sales s JOIN public.products p ON s.product_id = p.product_id JOIN public.customers c ON s.customer_id = c.customer_id 
WHERE s.sale_date >= '2025-04-01' AND s.sale_date < '2025-07-01' AND s.sales_type != '반품' AND 
sale_amount > (SELECT avg(sale_amount) FROM public.sales s WHERE s.sale_date >= '2025-04-01' 
AND s.sale_date < '2025-07-01' AND s.sales_type != '반품')
...
-- 선택 심화 SQL을 작성하세요.
SELECT sale_id, sale_date, product_name, customer_name, customer_type, customer_region, sales_type, quantity, sale_amount, payment_status,
round((SELECT avg(sale_amount) FROM public.sales s2 WHERE s2.sale_date >= '2025-04-01' AND s2.sale_date < '2025-07-01' AND s2.sales_type != '반품'),2) avg_sale_amount,
ROUND(s.sale_amount-(SELECT avg(s3.sale_amount) FROM public.sales s3 WHERE s3.sale_date >= '2025-04-01' AND s3.sale_date < '2025-07-01' AND s3.sales_type != '반품'),2) difference_from_avg
FROM public.sales s JOIN public.products p ON s.product_id = p.product_id JOIN public.customers c ON s.customer_id = c.customer_id 
WHERE s.sale_date >= '2025-04-01' AND s.sale_date < '2025-07-01' AND s.sales_type != '반품' AND 
sale_amount > (SELECT avg(sale_amount) FROM public.sales s WHERE s.sale_date >= '2025-04-01' 
AND s.sale_date < '2025-07-01' AND s.sales_type != '반품')
